    ...
    <div onClick={onClose} className={`fixed inset-0 flex justify-center items-end transition-colors z-[100] ${open ? 'visible bg-black/20' : 'invisible'} pt-10`}>
      <div onClick={(e) => e.stopPropagation()} className={`bg-gray-100 rounded-t-2xl drop-shadow-md transition-all duration-300 ${open ? 'h-[calc(100vh-40px)] opacity-100' : 'h-0 opacity-0'} w-screen`}>
        <div className="bg-white rounded-t-2xl p-5 w-full flex justify-between border">
          ...
          <button onClick={onClose} className="transition-colors p-1 border rounded-lg text-[#061834] hover:bg-gray-200 text:bg-black">
                  ...
                  {Object.entries(projectData.builtWith).map((e, i) => {
                    if (e[0] == key) {
                      return e[1].map((list, o) => (
                        <div key={o} className="flex border border-[#213B66]/40 text-[#061834] rounded-full w-fit px-5 py-3 font-bold">
                          {list}
                        </div>
                      ))
                    }
      ...
      <a onClick={(e) => e.stopPropagation()} href={projectData.projectUrl} target="_blank" className={`${open ? 'opacity-100' : 'opacity-0'} transition-all duration-300 fixed z-10 bottom-10 right-10 p-4 rounded-lg bg-[#213B66] text-[#E0ECFF] hover:bg-[#061834] hover:text-white hover:cursor-pointer`}>
