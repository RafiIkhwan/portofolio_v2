...
export default function Modal({ open, onClose, projectData, ref, customStyle }) {
	return (
		<div onClick={onClose} className={`fixed inset-0 flex justify-center items-end transition-colors z-[100] ${open ? "visible bg-black/20" : "invisible"} pt-10`}>
			<div onClick={(e) => e.stopPropagation()} className={`bg-light-gray rounded-t-2xl drop-shadow-md transition-all duration-300 ${open ? "h-[calc(100vh-40px)] opacity-100" : "h-0 opacity-0"} w-screen`}>
				<div className="bg-white rounded-t-2xl p-5 w-full flex justify-between border border-gray">
					<div className="flex flex-row space-x-4 items-center">
						<img src={Logo} className="h-8" alt="logo.png" draggable={false} />
						<h2 className="font-semibold">Project Detail</h2>
					</div>
					<button onClick={onClose} className="transition-colors p-1 border rounded-lg text-dark-blue hover:bg-light-gray text:bg-black">
				...
				<div ref={ref} id="modal" className="overflow-y-scroll h-full max-h-[calc(100vh-115px)]">
					<div className="flex flex-col gap-y-5 text-center py-10 max-w-6xl mx-auto">
						...
						<a href="https://github.com/rafiikhwan" className="flex self-center items-center space-x-2 group">
							<img src="https://github.com/rafiikhwan.png" alt="Profile.png" className="object-cover md:w-10 w-8 md:h-10 h-8 rounded-full" />
							<p className="relative inline-block lg:text-2xl md:text-xl text-lg font-semibold bg-gradient-to-r from-dark-blue to-dark-blue bg-[length:0%_1.6px] group-hover:bg-[length:100%_1.6px] bg-no-repeat bg-left-bottom transition-color duration-300">
								Rafi Ikhwan
								<span className="absolute inset-x-0 bottom-0 h-[1.6px] bg-gradient-to-r from-[#061834]/20 to-[#061834]/20 transition-all duration-400 group-hover:from-transparent"></span>
							</p>
						</a>
					</div>
			...
			{projectData.projectUrl && (
				<a onClick={(e) => e.stopPropagation()} href={projectData.projectUrl} target="_blank" className={`${open ? "opacity-100" : "opacity-0"} transition-all duration-300 fixed z-10 bottom-10 right-10 p-4 rounded-lg bg-blue text-gray hover:bg-dark-blue hover:text-white hover:cursor-pointer`}>
					<svg className="w-6 h-6 duration-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
