...
export default function About() {
	...
	const renderList = (index, label) => {
		...
		return (
			<li key={index} className={`${isActive ? "text-dark-blue bg-gradient-to-r from-dark-blue to-[#213b6671] bg-[length:100%_5px] drop-shadow-3xl" : `text-light-blue bg-gradient-to-r from-light-blue to-light-blue ${isHovered ? "bg-[length:100%_5px] drop-shadow-3xl" : "bg-[length:50%_5px]"}`} bg-no-repeat bg-left-bottom transition-color duration-150 font lg:text-2xl text-lg font-bold pb-3 cursor-pointer`} onClick={() => handleItemClick(index)} onMouseEnter={() => handleHover(index)} onMouseLeave={() => handleHover(null)}>
				{label}
			</li>
		);
	};
	...
	const renderItem = (index, title, content, context) => {
		return (
			<Fragment key={index}>
				{context != 0 ? (
					<li className="transition-colors duration-200 flex flex-row items-center rounded-3xl text-dark-blue lg:w-60 md:w-44 h-fit bg-gradient-to-r from-[#06183434] to-[#E0ECFF67] hover:cursor-pointer hover:bg-dark-blue hover:drop-shadow-2xl hover:text-light-blue drop-shadow group">
						{SVG_CALL[title]}
		...
		<>
			...
			<div className=" flex flex-row justify-between w-[calc(100vw-80px)] mx-auto md:space-x-20 md:w-full h-full">
				...
				<div className="flex flex-col space-y-8 md:space-y-16 z-30">
					...
					<p className="lg:text-3xl md:text-2xl text-[#CCDBF2] tracking-wider leading-normal text-justify md:mr-20">Hi, my name is Rafi Ikhwan, often called Rafi. Based in Bandung, Indonesia, I am proficient as a Full Stack Engineer, with a strong focus on Front-End Development and solid skills in Back-End Operations.</p>
