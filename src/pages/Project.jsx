...
export default function Project({ toggleModal, setView, projectData }) {
	return (
		<div className="flex flex-col justify-center h-full md:py-16 py-8">
			<h1 className="flex justify-center font-bold lg:text-8xl md:text-6xl text-3xl text-light-blue pb-8 md:pb-10">My Latest Project</h1>
			<Swiper
				slidesPerView={1}
				spaceBetween={0}
				pagination={{
			...
			>
              ...
              />
							<div className="absolute bottom-0 p-10 z-10 md:flex flex-col w-full lg:space-y-3 hidden">
								<h5 className="font-bold md:text-3xl lg:text-5xl text-light-blue">{data.projectTitle}</h5>
								<div className="flex flex-row justify-between items-center text-light-blue">
										...
										<li onClick={(e) => e.stopPropagation()}>
											{data.projectUrl && (
												<a href={data.projectUrl} title="Demo App">
													<svg className="w-6 h-6 hover:scale-105 hover:text-dark-blue transition-colors duration-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
